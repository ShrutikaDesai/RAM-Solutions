        ...
        <div style={{
          ...
          <div style={{ position: "relative", zIndex: 2, padding: isMobile ? "0" : "64px 56px 64px 60px", maxWidth: isMobile ? "100%" : 560 }}>
            ...
            <div style={{ marginBottom: spacing.xl }}>
              {["Right Analysis", "Matter", "Technology", "Pvt. Ltd."].map((line, i) => (
            ...
            <div style={{ display: "flex", gap: spacing.md, flexWrap: "wrap", opacity: heroIn ? 1 : 0, transition: "all 0.8s ease 0.68s" }}>
              <a href={WA} target="_blank" rel="noreferrer" style={{ background: colors.accent, color: colors.white, padding: "14px 30px", borderRadius: radius.md, textDecoration: "none", fontWeight: t.weight.extrabold, fontSize: t.size.sm, letterSpacing: "0.1em", textTransform: "uppercase", animation: "glowPulse 2.5s ease-in-out infinite" }}>
                Know More
              </a>
              <a href="#services" style={{ background: "transparent", color: colors.textOnDark85, padding: "14px 26px", borderRadius: radius.md, textDecoration: "none", fontWeight: t.weight.semibold, fontSize: t.size.sm, letterSpacing: "0.08em", textTransform: "uppercase", border: `1px solid ${colors.borderLight}` }}>
                Our Services ↓
              </a>
            </div>
          </div>
        ...
        <div style={{ flex: 1, background: colors.bgPage, display: "flex", flexDirection: "column", justifyContent: "center", padding: isMobile ? "40px 24px 52px" : "64px 48px 64px 80px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "center", gap: spacing.xl, opacity: heroIn ? 1 : 0, transition: "all 0.7s ease 0.95s", flexWrap: "wrap" }}>
            <div style={{ display: "flex" }}>
              {["AM", "RK", "PS"].map((av, i) => (
                <div key={i} style={{ width: iconSize.avatarSm, height: iconSize.avatarSm, borderRadius: radius.full, background: gradients.avatar, border: `2px solid ${colors.bgPage}`, marginLeft: i > 0 ? -10 : 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: t.size.xs, fontWeight: t.weight.extrabold, color: colors.white }}>{av}</div>
              ))}
            </div>
            <div>
              <div style={{ fontSize: t.size.base, fontWeight: t.weight.bold, color: colors.textHeading }}>Trusted by 50+ businesses</div>
              <div style={{ fontSize: t.size.xs, color: colors.textMuted }}>across India and globally</div>
            </div>
            <div style={{ marginLeft: "auto", display: "flex", gap: 3 }}>
              {[1, 2, 3, 4, 5].map(s => (
                <svg key={s} width="14" height="14" viewBox="0 0 16 16"><path d="M8 1l1.8 4h4.2l-3.4 2.5 1.3 4.1L8 9.1l-3.9 2.5 1.3-4.1L2 5h4.2z" fill={colors.accent} /></svg>
              ))}
            </div>
          </div>
        </div>
